...
const $$id = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const doc = await writeClient.fetch(
    `*[_id == $id][0]{ _id, number, order, title, copy }`,
    { id }
  );
  if (!doc) return Astro2.redirect("/admin/ritual-steps");
  return renderTemplate`${renderComponent($$result, "AdminLayout", $$AdminLayout, { "title": `Edit Ritual Step: ${doc.number}` }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<div class="mb-8"> <a href="/admin/ritual-steps" class="text-sm text-charcoal/50 hover:text-charcoal transition">← Ritual Steps</a> <h1 class="mt-2 font-display text-3xl text-charcoal">Step ${doc.number}</h1> </div> ${error && renderTemplate`<div class="mb-6 rounded-xl border border-red-200 bg-red-50 px-5 py-3 text-sm text-red-700">${error}</div>`}<form method="POST" class="space-y-8"> <div class="rounded-[1.5rem] border border-charcoal/10 bg-white/70 p-7 shadow-[0_8px_30px_rgba(31,37,34,0.06)]"> <div class="flex gap-0 border-b border-charcoal/10 mb-7"> <button type="button" data-tab-btn="fr" class="admin-tab-btn active">🇫🇷 French</button> <button type="button" data-tab-btn="en" class="admin-tab-btn">🇬🇧 English</button> <button type="button" data-tab-btn="ar" class="admin-tab-btn">🇸🇦 Arabic</button> </div> <div data-tab-panel="fr" class="space-y-5"> <div class="admin-field"> <label>Title (FR)</label> <input name="title_fr" class="admin-input"${addAttribute(loc(doc.title, "fr"), "value")} required> </div> <div class="admin-field"> <label>Instructions (FR)</label> <textarea name="copy_fr" rows="4" class="admin-input">${loc(doc.copy, "fr")}</textarea> </div> </div> <div data-tab-panel="en" class="hidden space-y-5"> <div class="admin-field"> <label>Title (EN)</label> <input name="title_en" class="admin-input"${addAttribute(loc(doc.title, "en"), "value")}> </div> <div class="admin-field"> <label>Instructions (EN)</label> <textarea name="copy_en" rows="4" class="admin-input">${loc(doc.copy, "en")}</textarea> </div> </div> <div data-tab-panel="ar" class="hidden space-y-5"> <div class="admin-field"> <label>Title (AR)</label> <input name="title_ar" class="admin-input" dir="rtl"${addAttribute(loc(doc.title, "ar"), "value")}> </div> <div class="admin-field"> <label>Instructions (AR)</label> <textarea name="copy_ar" rows="4" class="admin-input" dir="rtl">${loc(doc.copy, "ar")}</textarea> </div> </div> </div> <div class="rounded-[1.5rem] border border-charcoal/10 bg-white/70 p-7 shadow-[0_8px_30px_rgba(31,37,34,0.06)]"> <h2 class="mb-5 text-xs font-medium uppercase tracking-wider text-charcoal/50">Step Label</h2> <div class="admin-field max-w-xs"> <label>Step Number / Label</label> <input name="number" class="admin-input"${addAttribute(doc.number, "value")} placeholder="01"> </div> </div> <div class="flex justify-end gap-4"> <a href="/admin/ritual-steps" class="rounded-xl border border-charcoal/15 px-6 py-2.5 text-sm text-charcoal/60 hover:text-charcoal transition">Cancel</a> <button type="submit" class="rounded-xl bg-charcoal px-8 py-2.5 text-sm font-medium text-cream transition hover:bg-charcoal/80">Save Changes</button> </div> </form> ` })}`;
}, "C:/Users/WIN-11/Desktop/client website/klarite/src/pages/admin/ritual-steps/[id].astro", void 0);

const $$file = "C:/Users/WIN-11/Desktop/client website/klarite/src/pages/admin/ritual-steps/[id].astro";
const $$url = "/admin/ritual-steps/[id]";
